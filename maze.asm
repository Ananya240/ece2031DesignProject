...
; Currently Travles in basic U formation
STATE_1:
...
; Subroutine for moving forward
; Moves forward distance specified by dist variable
MoveForward:
...
TurnLeftRec:
	...
	ADDI   -1          ; check if within 1 degrees of target angle
	JPOS   TurnLeftRec       ; if not, keep testing
	CALL   WAIT1
	OUT    RESETPOS
	RETURN
...
TurnRightRec:
	...
	OUT    LCD         ; Display |angle error| for debugging
	ADDI   -1          ; check if within 1 degrees of target angle
	JPOS   TurnRightRec       ; if not, keep testing
	CALL   WAIT1
	OUT    RESETPOS
	RETURN

; Subroutine for turning around
...
TurnAroundRec:
	...
	CALL   Abs         ; absolute value subroutine
	OUT    LCD         ; Display |angle error| for debugging
	ADDI   -1          ; check if within 5 degrees of target angle
	JPOS   TurnAroundRec       ; if not, keep testing
	CALL   WAIT1
	OUT    RESETPOS
	RETURN
...
DIST:	  DW 0 ; use as argument for moveForward
;***************************************************************
;* Constants
